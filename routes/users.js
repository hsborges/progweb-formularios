...
const FIELDS = [
  {
    ...
    validator: function(value) {
      return value && validator.matches(value, /\d{4}.\d{4}.\d{3}-\d/gm);
    },
  },
  {
    field: 'nome',
  ...
  },
  {
    field: 'email',
    required: true,
    message: 'E se que quiser te mandar alguns spans ( ͡° ͜ʖ ͡°)',
  ...
  },
  {
    ...
    validator: function(value) {
      return CURSOS.reduce(
        (m, c) => m || value.trim().toLowerCase() === c.trim().toLowerCase(),
        false,
      );
    },
  },
  ...
  },
  {
    ...
    format: 'dd/mm/aaaa',
    validator: function(value) {
      return validator.matches(value, /\d{2}\/\d{2}\/\d{4}/gm);
    },
  },
  {
    field: 'telefone',
    ...
    validator: function(value) {
      return validator.matches(value, /\(\d{2,3}\) \d{5}-\d{4}/gm);
    },
  },
  {
    field: 'termos',
  ...
  router.post('/delete', async function(req, res, next) {
    db.users.remove({ _id: req.body.id }, function(err, n) {
      return res.status(400).render('users-feedback', {
        title:
          n > 0 ? 'Usuário removido com sucesso!' : 'Usuário não encontrado!',
        error: { removed: n > 0 },
        partials: { head: 'head' },
      });
    });
  ...
  router.post('/', async function(req, res, next) {
    ...
    db.users.findOne({ email: req.body.email }, async function(err, doc) {
      ...
      FIELDS.forEach(function(element) {
        ...
        if (!(typeof value == 'string') || !element.validator(value)) {
      ...
      db.users.insert(req.body, (err, doc) => {
        ...
        db.users.count({}, function(err, count) {
          if (err) {
            return res
              .status(500)
              .render('error', { message: 'Database error!', error: err });
          }
